...
let MAX_G = 2;
let F = 0.8;
let COLOR = ["#FE9D01", "#049FF1", "#43A102", "#EF1063", "#F60", "#A73800", "#EF1063", "#4C4C4C"];
...
window.onload = function () {
    ...
    let timer = setInterval(function () {
        ...
        for (let i = 0, len = balls.length; i < len; i++) {
            if (balls[i] && balls[i].x && (balls[i].x >= CANVAS_WIDTH || balls[i].x <= 0)) {
                balls.splice(i, 1);
            }
        }
    }, 30);
...
function update() {
    ...
    if (nextSecond !== curSecond) {
        if (nextHour / 10 !== curHour / 10) {
            addBalls(MARGIN_LEFT, MARGIN_TOP, curHour / 10);
        }
        if (nextHour % 10 !== curHour % 10) {
            addBalls(MARGIN_LEFT + 16 * (RADIUS + 1), MARGIN_TOP, curHour % 10);
        }
        if (nextMinute / 10 !== curMinute / 10) {
            addBalls(MARGIN_LEFT + 40 * (RADIUS + 1), MARGIN_TOP, curMinute / 10);
        }
        if (nextMinute % 10 !== curMinute % 10) {
            addBalls(MARGIN_LEFT + 56 * (RADIUS + 1), MARGIN_TOP, curMinute % 10);
        }
        if (nextSecond / 10 !== curSecond / 10) {
            addBalls(MARGIN_LEFT + 80 * (RADIUS + 1), MARGIN_TOP, curSecond / 10);
        }
        if (nextSecond % 10 !== curSecond % 10) {
            addBalls(MARGIN_LEFT + 96 * (RADIUS + 1), MARGIN_TOP, curSecond % 10);
        }
    }
}
...
function getCurrentTime() {
    ...
    let time = (endTime.getTime() - current.getTime()) / 1000;
...
function render(context) {
    ...
    renderDigit(context, 0, 0, hours / 10);
    renderDigit(context, 16 * (RADIUS + 1), 0, hours % 10);
    renderDigit(context, 32 * (RADIUS + 1), 0, 10);
    renderDigit(context, 40 * (RADIUS + 1), 0, minutes / 10);
    renderDigit(context, 56 * (RADIUS + 1), 0, minutes % 10);
    renderDigit(context, 72 * (RADIUS + 1), 0, 10);
    renderDigit(context, 80 * (RADIUS + 1), 0, seconds / 10);
    renderDigit(context, 96 * (RADIUS + 1), 0, seconds % 10);
