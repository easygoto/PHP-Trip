...
Baby.prototype.draw = function() {
  ...
  ctx1.drawImage(
    babyTail[tailCount],
    23 - 0.5 * babyTail[tailCount].width,
    -0.5 * babyTail[tailCount].height
  );
  let bodyCount = this.bodyCount;
  ctx1.drawImage(
    babyBody[bodyCount],
    -0.5 * babyBody[bodyCount].width,
    -0.5 * babyBody[bodyCount].height
  );
  let eyeCount = this.eyeCount;
  ctx1.drawImage(
    babyEye[eyeCount],
    -0.5 * babyEye[eyeCount].width,
    -0.5 * babyEye[eyeCount].height
  );
  ctx1.restore();
};
