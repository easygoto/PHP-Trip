...
Mom.prototype.draw = function () {
    ...
    ctx1.drawImage(momTail[tailCount], 30 - 0.5 * momTail[tailCount].width, -0.5 * momTail[tailCount].height);
    let bodyCount = this.bodyCount;
    if (data.double === 1) {
        ctx1.drawImage(momBodyOra[bodyCount], -0.5 * momBodyOra[bodyCount].width, -0.5 * momBodyOra[bodyCount].height);
    } else {
        ctx1.drawImage(
    ...
    ctx1.drawImage(momEye[eyeCount], -0.5 * momEye[eyeCount].width, -0.5 * momEye[eyeCount].height);
    ctx1.restore();
};
